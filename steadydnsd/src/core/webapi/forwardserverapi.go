...
func forwardServerHandler(w http.ResponseWriter, r *http.Request) {
	...
	case 3: // /api/forward-servers/{id}
		...
		if err != nil {
			sendErrorResponse(w, "无效的服务器ID", http.StatusBadRequest)
			return
		}
...
func getForwardServerByID(w http.ResponseWriter, serverID uint) {
	...
	if err != nil {
		sendErrorResponse(w, fmt.Sprintf("获取服务器失败: %v", err), http.StatusNotFound)
		return
	}

	sendSuccessResponse(w, server, "获取服务器成功")
}
...
func updateForwardServer(w http.ResponseWriter, r *http.Request, serverID uint) {
	...
	if err := database.UpdateDNSServer(&server); err != nil {
		if strings.Contains(err.Error(), "服务器不存在") {
			sendErrorResponse(w, fmt.Sprintf("更新服务器失败: %v", err), http.StatusNotFound)
		} else {
			sendErrorResponse(w, fmt.Sprintf("更新服务器失败: %v", err), http.StatusInternalServerError)
		}
		return
	}
	...
	sendSuccessResponse(w, server, "服务器更新成功")
}
...
func deleteForwardServer(w http.ResponseWriter, serverID uint) {
	...
	if err != nil {
		sendErrorResponse(w, "服务器不存在", http.StatusNotFound)
		return
	}

	// 删除服务器
	if err := database.DeleteDNSServer(serverID); err != nil {
		sendErrorResponse(w, fmt.Sprintf("删除服务器失败: %v", err), http.StatusInternalServerError)
		return
	}
	...
	sendSuccessResponse(w, nil, "服务器删除成功")
}
...
func batchAddForwardServers(w http.ResponseWriter, r *http.Request) {
	...
	if len(servers) == 0 {
		sendErrorResponse(w, "服务器列表不能为空", http.StatusBadRequest)
		return
	}
	...
	sendSuccessResponse(w, response, fmt.Sprintf("成功添加 %d 个服务器", successCount))
}
...
func batchDeleteForwardServers(w http.ResponseWriter, r *http.Request) {
	...
	sendSuccessResponse(w, response, fmt.Sprintf("成功删除 %d 个服务器", successCount))
}
