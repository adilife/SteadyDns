...
func forwardGroupHandler(w http.ResponseWriter, r *http.Request) {
	...
	case 3: // /api/forward-groups/{id}
		...
		if err != nil {
			...
			sendErrorResponse(w, "无效的转发组ID", http.StatusBadRequest)
			return
		}
...
func getForwardGroups(w http.ResponseWriter) {
	...
	if err != nil {
		sendErrorResponse(w, fmt.Sprintf("获取转发组列表失败: %v", err), http.StatusInternalServerError)
		return
	}
	...
	sendSuccessResponse(w, groups, "获取转发组列表成功")
}
...
func createForwardGroup(w http.ResponseWriter, r *http.Request) {
	...
	if err := database.ValidateForwardGroupDB(&group); err != nil {
		sendErrorResponse(w, fmt.Sprintf("验证失败: %v", err), http.StatusBadRequest)
		return
	}
	...
	if err == nil && existingGroup != nil {
		sendErrorResponse(w, "转发组域名已存在", http.StatusBadRequest)
		return
	}

	// 创建转发组
	if err := database.CreateForwardGroup(&group); err != nil {
		sendErrorResponse(w, fmt.Sprintf("创建转发组失败: %v", err), http.StatusInternalServerError)
		return
	}
	...
	sendSuccessResponse(w, group, "转发组创建成功")
}
...
func updateForwardGroup(w http.ResponseWriter, r *http.Request, groupID uint) {
	...
	if groupID == 1 {
		...
		if err != nil {
			sendErrorResponse(w, fmt.Sprintf("获取转发组失败: %v", err), http.StatusNotFound)
			return
		}
		// 检查是否尝试修改域名或描述
		if group.Domain != currentGroup.Domain || group.Description != currentGroup.Description {
			sendErrorResponse(w, "ID=1的转发组是默认组，域名和描述不可修改", http.StatusBadRequest)
			return
		}
	}
	...
	if err := database.UpdateForwardGroup(&group); err != nil {
		if strings.Contains(err.Error(), "转发组不存在") {
			sendErrorResponse(w, fmt.Sprintf("更新转发组失败: %v", err), http.StatusNotFound)
		} else {
			sendErrorResponse(w, fmt.Sprintf("更新转发组失败: %v", err), http.StatusInternalServerError)
		}
		return
	}
	...
	sendSuccessResponse(w, group, "转发组更新成功")
}
...
func deleteForwardGroup(w http.ResponseWriter, groupID uint) {
	...
	if groupID == 1 {
		sendErrorResponse(w, "ID=1的转发器组是默认组，不允许删除", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		sendErrorResponse(w, "转发组不存在", http.StatusNotFound)
		return
	}

	// 删除转发组
	if err := database.DeleteForwardGroup(groupID); err != nil {
		sendErrorResponse(w, fmt.Sprintf("删除转发组失败: %v", err), http.StatusInternalServerError)
		return
	}

	sendSuccessResponse(w, nil, "转发组删除成功")
}
...
func getForwardGroupByID(w http.ResponseWriter, groupID uint) {
	...
	sendSuccessResponse(w, group, "获取转发组成功")
}
...
func batchDeleteForwardGroups(w http.ResponseWriter, r *http.Request) {
	...
	if len(ids) == 0 {
		sendErrorResponse(w, "ID列表不能为空", http.StatusBadRequest)
		return
	}
	...
	if err := database.BatchDeleteForwardGroups(ids); err != nil {
		sendErrorResponse(w, fmt.Sprintf("批量删除转发组失败: %v", err), http.StatusInternalServerError)
		return
	}

	sendSuccessResponse(w, nil, fmt.Sprintf("成功删除 %d 个转发组", len(ids)))
}
...
func testDomainMatchHandler(w http.ResponseWriter, r *http.Request) {
	...
	if domain == "" {
		sendErrorResponse(w, "域名参数不能为空", http.StatusBadRequest)
		return
	}
